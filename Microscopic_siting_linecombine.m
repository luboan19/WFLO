...
Constraints=[sum(y)==19,p>=0,v_rate>=0,v_fang>=0,v_san>=0];  %约束，总机数
for dd=1:D
...
for dd=1:D
    for jj= 1:I
    Constraints=[Constraints,v_rate(jj,dd) <=(1- y'*DD(:,jj,dd))];  
...
ops = sdpsettings('solver', 'gurobi','verbose', 2,'gurobi.NonConvex',2,'gurobi.Timelimit',600,'usex0',1);%设置求解器
